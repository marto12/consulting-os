...
- **2026-02-18**: Enhanced Dataset management — datasets now support CSV upload (drag-and-drop with parsing), API endpoint linking, and tabular data preview with pagination. Dataset cards show source type badges (CSV/API/Manual), row/column counts. Full CRUD: create, edit, delete datasets. CSV data stored in `dataset_rows` table. Backend: `PUT /api/data/datasets/:id`, `DELETE /api/data/datasets/:id`, `POST /api/data/datasets/:id/upload-csv`, `GET /api/data/datasets/:id/rows`. Schema additions: `sourceType`, `sourceUrl`, `rowCount` on datasets; new `dataset_rows` table.
- **2026-02-18**: Added Workflow Template Editor — visual React Flow graph interface for creating and editing workflow templates. Network graph shows pipeline steps as connected nodes. Right panel has step cards with name, agent dropdown, description, and reorder/delete controls. Workflows page now has New Workflow, Edit, Duplicate, and Delete actions. Routes: `/workflow/new`, `/workflow/:id`. Backend: `DELETE /api/workflows/:id`, `PUT /api/workflows/:id` (bulk step replace).
- **2026-02-18**: Added Key Narrative agent for word processor — scans technical prose and extracts executive-level key points with narrative role labels (Core thesis, Supporting evidence, Risk/caveat, Action driver, Context setter). Proposes crisp executive-ready rewrites. Teal-styled comments with lightbulb icon in sidebar. Endpoint: `POST /api/documents/:id/narrative-review`.
- **2026-02-18**: Fixed document agent text positioning — agents now strip HTML before sending to LLM, request exact quoted text, and use HTML-to-ProseMirror position mapper to calculate correct editor positions. Fixes word fragment highlighting issue across all review agents.
