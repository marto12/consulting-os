...
export const workflowTemplates = pgTable("workflow_templates", {
  ...
  practiceCoverage: jsonb("practice_coverage").default(sql`'[]'::jsonb`).notNull(),
  timesUsed: integer("times_used").notNull().default(0),
  deploymentStatus: text("deployment_status").notNull().default("sandbox"),
  governanceMaturity: integer("governance_maturity").notNull().default(1),
  baselineCost: real("baseline_cost"),
  aiCost: real("ai_cost"),
  lifecycleStatus: text("lifecycle_status").notNull().default("active"),
  comingSoonEta: text("coming_soon_eta"),
...
export const projects = pgTable("projects", {
  ...
  workflowTemplateId: integer("workflow_template_id"),
  governanceControls: jsonb("governance_controls").notNull().default(sql`'{}'::jsonb`),
  totalSavingsToDate: real("total_savings_to_date"),
  costReductionRealisedPct: real("cost_reduction_realised_pct"),
  marginImpactToDate: real("margin_impact_to_date"),
  projectedAnnualImpact: real("projected_annual_impact"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
  updatedAt: timestamp("updated_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
});
...
export const projectTasks = pgTable("project_tasks", {
  ...
  description: text("description").notNull().default(""),
  taskType: text("task_type").notNull().default("human"),
