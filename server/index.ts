...
async function setupViteDevMiddleware(app: express.Application) {
...
function serveProductionFrontend(app: express.Application) {
    ...
    const startProxy = () => {
      ...
      proxyServer.on("error", (err: NodeJS.ErrnoException) => {
        if (err.code === "EADDRINUSE") {
          ...
          try { execSync(`fuser -k ${devProxyPort}/tcp 2>/dev/null || true`); } catch {}
